...
for (let key of Object.keys(primitiveTypes)) {
	if (primitiveTypes[key]) {
		if (!primitiveTypes[key].validator) {
			primitiveTypes[key].validator = () => true;
		}
		if (!primitiveTypes[key].regex) {
			primitiveTypes[key].regex = { test: () => true };
		}
	}
}
